...
def _make_random_qmf(size, precision):
    ...
    qmf = np.bincount(rng.choice(size, 2**precision - size, p=pmf), minlength=size)
...
def test_encode_decode(n_distribution, with_fallback):
    ...
    for i in range(n_distribution):
        ...
        alphabet_size = rng.integers(257, min(2**precision, 4096), endpoint=True)

        qmfs.append(_make_random_qmf(alphabet_size, precision))

        data = np.where(index != i, data, rng.integers(0, alphabet_size, data.shape, dtype=np.int32))
...
def test_error_index_and_value_mismatch(with_fallback):
    ...
    with pytest.raises(range_ans_pybind.AnsError) as cm:
        if with_fallback:
            stack.push_with_fallback(encoder, np.zeros(3, dtype=np.int32), np.zeros(2, dtype=np.int32))
        else:
            stack.push(encoder, np.zeros(3, dtype=np.int32), np.zeros(2, dtype=np.int32))
