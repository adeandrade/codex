...
class RangeAnsStack:
    ...
    @classmethod
    def make(cls, stream: chex.Array, initial_state: ... = 0) -> RangeAnsStack:
    ...
    def _write(self, n_bytes: int) -> RangeAnsStack:
        ...
        stream = lax.dynamic_update_slice_in_dim(self.stream, update.astype(jnp.uint8), self.stream_loc, axis=-1)
    ...
    @classmethod
    def deserialize(cls, stream: chex.Array, stream_size: chex.Array) -> RangeAnsStack:
        ...
        stack = cls(state=jnp.zeros((), dtype=jnp.uint32), stream=stream, stream_loc=stream_size)
        # REQUIRES: 3 <= stream_size and stream_size <= stream.size
        # rANS state is uint32 and 3 or 4 bytes are going to be read.
        return lax.cond(stream_size & 1 == 0, lambda s: s._read(4), lambda s: s._read(3), stack)  # pylint: disable=protected-access

    def decode_scalar(self, cdf: chex.Array, precision: chex.Array) -> tuple[RangeAnsStack, jax.Array]:
        ...
        stack = lax.cond(
            jnp.logical_and(self.stream_loc != 0, self.state < (1 << LOG2_MIN_STATE)),
            lambda s: s.read(2),
            lambda s: s,
            self,
        )
    ...
    def encode_scalar(self, bound: tuple[chex.Array, chex.Array], precision: chex.Array | int) -> RangeAnsStack:
        ...
        stack = lax.cond(size <= (self.state >> (LOG2_MAX_STATE - precision)), lambda s: s.write(2), lambda s: s, self)
    ...
    def decode(
        ...
        index: chex.Array,
        cdf: chex.Array,
        precision: chex.Array | int,
    ) -> tuple[RangeAnsStack, jax.Array]:
        ...
        if cdf.ndim == 1:
            ...
            return lax.scan(lambda s, _: s.decode_scalar(cdf, precision), self, (), length=index.shape[-1])
        else:
            return lax.scan(lambda s, i: s.decode_scalar(cdf[i], precision), self, index)

    def encode(
        self,
        value: tuple[chex.Array, chex.Array] | tuple[chex.Array, chex.Array, chex.Array],
        precision: chex.Array | int,
    ) -> RangeAnsStack:
    ...
    def pop_scalar(self, decoder: chex.Array, index: chex.Array | int) -> tuple[RangeAnsStack, jax.Array]:
    ...
    def pop(self, decoder: chex.Array, index: chex.Array) -> tuple[RangeAnsStack, jax.Array]:
    ...
    def push_scalar(self, encoder: chex.Array, index: chex.Array | int, value: chex.Array | int) -> RangeAnsStack:
    ...
    def push(self, encoder: chex.Array, index: chex.Array, value: chex.Array) -> RangeAnsStack:
...
def _range_ans_stack_make_encoder(decoder: np.ndarray) -> np.ndarray:
    ...
    for precision, shift in zip(precision, shift):
        ...
        np.testing.assert_array_equal(np.sort(lookup), np.arange(lookup.size, dtype=lookup.dtype))
...
def quantize_distribution(pmf: chex.Array, normalizer: int) -> chex.Array:
    ...
    if pmf.size == 0:
        return -jnp.ones_like(pmf, dtype=jnp.int32)
