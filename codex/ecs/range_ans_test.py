...
def _make_random_quantized_cdf(shape, precision):
    *shape, n = shape
    arr = np.stack([_make_random_quantized_cdf_1d(n, precision) for _ in range(math.prod(shape))])
...
def _check_encoded_size(encoded, cdf, x, index=None):
    ...
    for optimal_size, stream_size, ratio in zip(*np.atleast_1d(optimal_size, stream_size, ratio)):
        logging.info(
    ...
    np.testing.assert_array_less(stream_size * (15.0 < optimal_size), 1 + np.ceil(1.1 * optimal_size))
...
def test_encode_and_decode_python_loop(m, n):
    ...
    encode_single_fn = jax.jit(functools.partial(range_ans.RangeAnsStack.encode_scalar, precision=precision))
    decode_single_fn = jax.jit(functools.partial(range_ans.RangeAnsStack.decode_scalar, precision=precision))
